...
def most_similar(sen, voting_dict, largest=False):
    """
    ...
    """
    equality = list()
    for s, lst in voting_dict.items():
        if s == sen:
            continue
        equality.append((s, policy_compare(sen, s, voting_dict)))
    if largest:
        r = min(*equality, key=lambda x: x[1])
    else:
        r = max(*equality, key=lambda x: x[1])
    return r[0]
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'c'
    """
    return most_similar(sen, voting_dict, largest=True)
...
if __name__ == '__main__':
    #print(create_voting_dict(['Kennedy D MA -1 -1 1 1', 'Snowe R ME 1 1 1 1']))
    #vd = {'Klein': [1,1,1], 'Fox-Epstein': [1,-1,0], 'Ravella': [-1,0,0]}
    #print(most_similar('Klein', vd, largest=False))

    vd = create_voting_dict(['Kennedy D MA -1 -1 1 1', 'Snowe R ME 1 1 1 1'])
    print(vd == {'Snowe': [1, 1, 1, 1], 'Kennedy': [-1, -1, 1, 1]})